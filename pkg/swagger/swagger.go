...
import (
	...
	"gitlab.com/tellmecomua/tellme.api/pkg/version"
)

func GetApidocsJSON() ([]byte, error) {
	return (&swagger{ws: &restful.WebService{}}).genSwagger()
}
...
func (s *swagger) genSwagger() ([]byte, error) {
	s.init()
	...
	if err != nil {
		return nil, fmt.Errorf("failed to marshal swagger: %v", err)
	}
